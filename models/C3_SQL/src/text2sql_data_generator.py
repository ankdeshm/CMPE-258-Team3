...
def parse_option():
    ...
    parser.add_argument('--input_dataset_path', type=str, default="/content/drive/MyDrive/CMPE_258/C3SQL-master/data/pre-processing/dev_with_probs.json",
                        help='filepath of the input dataset.')
    parser.add_argument('--output_dataset_path', type=str, default="/content/drive/MyDrive/CMPE_258/C3SQL-master/data/pre-processing/resdsql_dev.json",
    ...
    parser.add_argument("--target_type", type=str, default="sql",
                        help="sql or natsql.")
    # parser.add_argument("--instruction_task", type=str, default="normal")
    parser.add_argument("--instruction_tasks", type=str, default=["thu_prompt"], nargs="+")
...
def prepare_input_and_output_thu_prompt(opt, ranked_data):
    question = ranked_data["question"]

    schema_sequence = ""
    for table_id in range(len(ranked_data["db_schema"])):
        table_name_original = ranked_data["db_schema"][table_id]["table_name_original"]
        # schema_sequence += f"# {table_name_original} ( "
        ...
        columns = " , ".join(column_info_list)
        schema_sequence += f"# {table_name_original} ( {columns} )\n"

    if opt.add_fk_info:
        for fk in ranked_data["fk"]:
            schema_sequence += f"# {fk['source_table_name_original']}.{fk['source_column_name_original']} = {fk['target_table_name_original']}.{fk['target_column_name_original']}\n"
    ...
    # instruction = f"### Complete sqlite SQL query only and with no explanation\n### Sqlite SQL tables, with their properties: \n# \n"
    instruction = f"### Complete sqlite SQL query only and with no explanation, and do not select extra columns that are not explicitly requested in the query, \n### Sqlite SQL tables, with their properties: \n# \n"
    input_sequence = f"{instruction}{schema_sequence}#\n### {question}\nSELECT"
...
def prepare_input_and_output_generate_skeleton(opt, ranked_data):
    question = ranked_data["question"]
    ...
    instruction = f"### Complete sqlite SQL query, and replace the columns and tables with __. Notice you only need to output the sql skeleton.\n### Sqlite SQL tables, with their properties: \n# \n"
    input_sequence = f"{instruction}{schema_sequence}### {question}\n"

    if opt.target_type == "sql":
        output_sequence = ranked_data["sql_skeleton"]
    elif opt.target_type == "natsql":
        output_sequence = ranked_data["natsql_skeleton"]

    return input_sequence, output_sequence

def prepare_input_and_output_fill_skeleton(opt, ranked_data):
    question = ranked_data["question"]
    ...
    instruction = f"### Fill the blanks in the SQL skeleton to complete sqlite SQL query.\n### Sqlite SQL tables, with their properties: \n# \n"
    input_sequence = f"{instruction}{schema_sequence}### {question}\n"

    if opt.target_type == "sql":
        output_sequence = ranked_data["norm_sql"]
        skeleton = ranked_data["sql_skeleton"]
    elif opt.target_type == "natsql":
        output_sequence = ranked_data["norm_natsql"]
        skeleton = ranked_data["natsql_skeleton"]

    input_sequence = f"{instruction}{schema_sequence}### {question}\n### {skeleton}\n"

    return input_sequence, output_sequence

def prepare_input_and_output_predict_schema_items(opt, ranked_data):
    pass
...
def generate_train_ranked_dataset(opt):
    ...
    for data_id, data in enumerate(dataset):
        ...
        for task in opt.instruction_tasks:

            prepare_function = prepare_function_map[task]
            input_sequence, output_sequence = prepare_function(opt, ranked_data)
...
prepare_function_map = {
    "normal": prepare_input_and_output,
    "thu_prompt": prepare_input_and_output_thu_prompt,
    "fill_skeleton": prepare_input_and_output_fill_skeleton,
    "generate_skeleton": prepare_input_and_output_generate_skeleton,
}
