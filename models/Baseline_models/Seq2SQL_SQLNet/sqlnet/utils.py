...
def load_dataset(dataset_id, use_small=False):
    if dataset_id == 0:
        ...
        sql_data, table_data = load_data('/content/drive/MyDrive/258/SQLNet/sqlnet/data/train_tok.jsonl',
                '/content/drive/MyDrive/258/SQLNet/sqlnet/data/train_tok.tables.jsonl', use_small=use_small)
        val_sql_data, val_table_data = load_data('/content/drive/MyDrive/258/SQLNet/sqlnet/data/dev_tok.jsonl',
                '/content/drive/MyDrive/258/SQLNet/sqlnet/data/dev_tok.tables.jsonl', use_small=use_small)
        test_sql_data, test_table_data = load_data('/content/drive/MyDrive/258/SQLNet/sqlnet/data/test_tok.jsonl',
                '/content/drive/MyDrive/258/SQLNet/sqlnet/data/test_tok.tables.jsonl', use_small=use_small)
        TRAIN_DB = '/content/drive/MyDrive/258/SQLNet/sqlnet/data/train.db'
        DEV_DB = '/content/drive/MyDrive/258/SQLNet/sqlnet/data/dev.db'
        TEST_DB = '/content/drive/MyDrive/258/SQLNet/sqlnet/data/test.db'
    else:
        print("Loading from re-split dataset")
        sql_data, table_data = load_data('/content/drive/MyDrive/258/SQLNet/sqlnet/data_resplit/train.jsonl',
                '/content/drive/MyDrive/258/SQLNet/sqlnet/data_resplit/tables.jsonl', use_small=use_small)
        val_sql_data, val_table_data = load_data('/content/drive/MyDrive/258/SQLNet/sqlnet/data_resplit/dev.jsonl',
                '/content/drive/MyDrive/258/SQLNet/sqlnet/data_resplit/tables.jsonl', use_small=use_small)
        test_sql_data, test_table_data = load_data('/content/drive/MyDrive/258/SQLNet/sqlnet/data_resplit/test.jsonl',
                '/content/drive/MyDrive/258/SQLNet/sqlnet/data_resplit/tables.jsonl', use_small=use_small)
        TRAIN_DB = '/content/drive/MyDrive/258/SQLNet/sqlnet/data_resplit/table.db'
        DEV_DB = '/content/drive/MyDrive/258/SQLNet/sqlnet/data_resplit/table.db'
        TEST_DB = '/content/drive/MyDrive/258/SQLNet/sqlnet/data_resplit/table.db'


    return sql_data, table_data, val_sql_data, val_table_data,test_sql_data, test_table_data, TRAIN_DB, DEV_DB, TEST_DB
...
def best_model_name(args, for_load=False):
    ...
    agg_model_name = '/content/drive/MyDrive/258/SQLNet/saved_model/%s_%s%s%s.agg_model'%(new_data,
            mode, use_emb, use_ca)
    sel_model_name = '/content/drive/MyDrive/258/SQLNet/saved_model/%s_%s%s%s.sel_model'%(new_data,
            mode, use_emb, use_ca)
    cond_model_name = '/content/drive/MyDrive/258/SQLNet/saved_model/%s_%s%s%s.cond_%smodel'%(new_data,
            mode, use_emb, use_ca, use_rl)

    if not for_load and args.train_emb:
        agg_embed_name = '/content/drive/MyDrive/258/SQLNet/saved_model/%s_%s%s%s.agg_embed'%(new_data,
                mode, use_emb, use_ca)
        sel_embed_name = '/content/drive/MyDrive/258/SQLNet/saved_model/%s_%s%s%s.sel_embed'%(new_data,
                mode, use_emb, use_ca)
        cond_embed_name = '/content/drive/MyDrive/258/SQLNet/saved_model/%s_%s%s%s.cond_embed'%(new_data,
                mode, use_emb, use_ca)
...
def epoch_train(model, optimizer, batch_size, sql_data, table_data, pred_entry):
    ...
    while st < len(sql_data):
        ...
        cum_loss += loss.item() * (ed - st)  # Change made here
        optimizer.zero_grad()
        loss.backward()
        optimizer.step()
...
def load_word_emb(file_name, load_used=False, use_small=False):
    if not load_used:
        print(f"Loading word embedding from {file_name}")
        ret = {}
        with open(file_name) as inf:
            for idx, line in enumerate(inf):
                if (use_small and idx >= 5000):
                    print("break in load_word_emb")
                    break
        ...
        w2i_path = '/content/drive/MyDrive/258/SQLNet/glove/word2idx.json'
        word_emb_val_path = '/content/drive/MyDrive/258/SQLNet/glove/usedwordemb.npy'
        with open(w2i_path, 'r') as inf:
            w2i = json.load(inf)
        # Open the .npy file in binary mode 'rb'
        with open(word_emb_val_path, 'rb') as inf:
            word_emb_val = np.load(inf)
        return w2i, word_emb_val
        
        # with open('/content/drive/MyDrive/258/SQLNet/glove/word2idx.json') as inf:
        #     w2i = json.load(inf)
        # with open('/content/drive/MyDrive/258/SQLNet/glove/usedwordemb.npy') as inf:
        #     word_emb_val = np.load(inf)
        # return w2i, word_emb_val
